...
public class ViewAPI<Entity> {
    ...
    public void create(){
        ...
        List<Field> fields = Arrays.stream(entity.getDeclaredFields())
                .filter(field -> (field.isAnnotationPresent(Column.class) ||field.isAnnotationPresent(JoinColumn.class)) && !field.isAnnotationPresent(Id.class))
    ...
    private void input(Field field, Entity entity) throws IllegalAccessException, SQLException {
        ...
        if (fieldType == Integer.class){
            ...
            final CountryController controllerCountry = new CountryController();
            Country country = controllerCountry.get(Integer.parseInt(input));
            field.set(entity, country);
        }else if (fieldType == City.class){
            final CityController controllerCity = new CityController();
            City city = controllerCity.get(Integer.parseInt(input));
            field.set(entity, city);
        } else if (fieldType == BankAccount.class){
            final BankAccountController controllerBankAccount = new BankAccountController();
            BankAccount bankAccount = controllerBankAccount.get(Integer.parseInt(input));
            field.set(entity, bankAccount);
        } else if (fieldType == Client.class){
            final ClientController controllerClient = new ClientController();
            Client client = controllerClient.get(Integer.parseInt(input));
            field.set(entity, client);
        }else if (fieldType == Item.class){
            final ItemController controllerItem = new ItemController();
            Item item = controllerItem.get(Integer.parseInt(input));
            field.set(entity, item);
        } else if (fieldType == Street.class){
            final StreetController controllerStreet = new StreetController();
            Street street = controllerStreet.get(Integer.parseInt(input));
            field.set(entity, street);
        } else if (fieldType == Transaction.class){
            final TransactionController controllerTransaction = new TransactionController();
            Transaction transaction = controllerTransaction.get(Integer.parseInt(input));
            field.set(entity, transaction);
        }
